...
public class RegistosProducaoServiceTests
{
    ...
    [Test(Description = "TRP005A - Fluxo industrial pode iniciar diretamente em EM_CURSO.")]
    public async Task CreateFromIndustrialEventAsync_Should_AcceptEmCurso_AsFirstState()
    {
        // ARRANGE
        SetupValidDependencies();
        SetupPersistCreated();
        ...
        // ACT
        var result = await _sut.CreateFromIndustrialEventAsync(BuildDto(estado: EstadoProducao.EM_CURSO, maquinaId: 1), DateTime.UtcNow, permitirPrimeiroEstadoEmCurso: true);
